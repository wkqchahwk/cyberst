...
func (h *MarkdownAgentsHandler) ListMarkdownAgents(c *gin.Context) {
	if h.dir == "" {
		c.JSON(http.StatusOK, gin.H{"agents": []any{}, "dir": "", "error": " agents "})
		return
	}
...
func (h *MarkdownAgentsHandler) CreateMarkdownAgent(c *gin.Context) {
	if h.dir == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": " agents "})
		return
	}
	...
	if want := agents.WantsMarkdownOrchestrator(filepath.Base(path), body.Kind, string(out)); want {
		...
		if other != "" {
			c.JSON(http.StatusConflict, gin.H{"error": fmt.Sprintf("：%s，", other)})
			return
		}
	}
	...
	c.JSON(http.StatusOK, gin.H{"filename": filepath.Base(path), "message": ""})
}
