...
func (db *DB) SaveToolExecution(exec *mcp.ToolExecution) error {
	...
	if err != nil {
		db.logger.Error("", zap.Error(err), zap.String("executionId", exec.ID))
		return err
	}
...
func (db *DB) DeleteToolExecution(id string) error {
	...
	if err != nil {
		db.logger.Error("", zap.Error(err), zap.String("executionId", id))
		return err
	}
	return nil
}
...
func (db *DB) DeleteToolExecutions(ids []string) error {
	...
	if err != nil {
		db.logger.Error("", zap.Error(err), zap.Int("count", len(ids)))
		return err
	}
	return nil
}
...
func (db *DB) SaveToolStats(toolName string, stats *mcp.ToolStats) error {
	...
	if err != nil {
		db.logger.Error("", zap.Error(err), zap.String("toolName", toolName))
		return err
	}
...
func (db *DB) DecreaseToolStats(toolName string, totalCalls, successCalls, failedCalls int) error {
	...
	if newTotalCalls == 0 {
		...
		if err != nil {
			db.logger.Warn("", zap.Error(err), zap.String("toolName", toolName))
			// English note.
		} else {
			db.logger.Info("", zap.String("toolName", toolName))
		}
	}
